...
export default (
  ...
  const locale = configuration.locale || "fr";
  ...
  if (!dictionary[locale]) {
    throw new Error(
      "No translation available for the target locale (relative dates)."
    );
  }
  ...
  //     configuration
  //   },
  //   'attempting to extract date from "%s" input',
  //   normalizedInput
  // );
    ...
    for (const movingChunk of movingChunks) {
      ...
      if (format.dateFnsFormat === "R") {
        if (!configuration.locale) {
          ...
          const maybeDate = extractRelativeDate(
            subject,
          ...
          if (maybeDate) {
            ...
            //log.debug(
            //   'matched "%s" input using "%s" format (%s direction)',
            //   subject,
        ...
        format.dateFnsFormat === "EEE" ||
        format.dateFnsFormat === "EEEE"
      ) {
        ...
        const yearIsExplicit =
          typeof format.yearIsExplicit === "boolean"
            ? format.yearIsExplicit
            : true;
        ...
        if (yearIsExplicit) {
          ...
          if (
            ...
            format.dateFnsFormat.includes("yyyy") &&
            formatDirection.replace("Y", "") ===
              configurationDirection.replace("Y", "")
          ) {
            //log.debug("matched format using yyyy; month-day direction matches");
          } else if (
            format.direction &&
            format.direction !== configuration.direction
          ) {
          ...
          if (format.direction && !configuration.direction) {
            //log.info(
            //   'found a match using "%s" format; unsafe to use without `direction` configuration',
            //   format.dateFnsFormat
            // );
          ...
          const currentMonth =
            parseInt(formatDate(baseDate, "M"), 10) + currentYear * 12;
          const parsedMonth =
            parseInt(formatDate(date, "M"), 10) +
            parseInt(formatDate(date, "yyyy"), 10) * 12;
          ...
          if (difference >= configuration.maximumAge) {
            ...
            difference < 0 &&
            Math.abs(difference) >= configuration.minimumAge
          ) {
            useYear = currentYear + 1;
          } else {
            useYear = currentYear;
          ...
          if (format.direction && !configuration.direction) {
            //log.info(
            //   'found a match using "%s" format; unsafe to use without "format" configuration',
            //   format.dateFnsFormat
            // );
